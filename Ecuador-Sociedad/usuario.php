            ...
            */
            ...
            if(isset($_FILES['archivo']) && $_FILES['archivo']['error'] === UPLOAD_ERR_OK) {
                ...
                if ($imageInfo !== false) {
                    ...
                    $sourceImage = null;
                    switch ($imageInfo[2]) { // $imageInfo[2] contiene el tipo de imagen
                        case IMAGETYPE_JPEG:
                            $sourceImage = imagecreatefromjpeg($_FILES['archivo']['tmp_name']);
                            break;
                        case IMAGETYPE_PNG:
                            $sourceImage = imagecreatefrompng($_FILES['archivo']['tmp_name']);
                            break;
                        case IMAGETYPE_GIF:
                            $sourceImage = imagecreatefromgif($_FILES['archivo']['tmp_name']);
                            break;
                    }
                    
                    if ($sourceImage) {
                        // Crear directorios si no existen
                        if (!file_exists("images/usuarios/thumbs")) {
                            mkdir("images/usuarios/thumbs", 0755, true);
                        }
                        if (!file_exists("images/usuarios/full")) {
                            mkdir("images/usuarios/full", 0755, true);
                        }

                        // Calcular dimensiones para el crop cuadrado
                        $srcWidth = imagesx($sourceImage);
                        $srcHeight = imagesy($sourceImage);
                        
                        // Determinar el lado más corto para hacer un cuadrado
                        ...
                        // 1. Guardar versión en alta calidad (800x800)
                        $largeImage = imagecreatetruecolor(800, 800);
                        imagecopyresampled($largeImage, $sourceImage,
                            0, 0, // Destino x, y
                            $cropX, $cropY, // Origen x, y (centro de la imagen)
                            800, 800, // Tamaño final deseado
                            $cropSize, $cropSize // Tamaño del área a copiar (cuadrado)
                        );
                        
                        // 2. Crear miniatura (200x200)
                        $thumbImage = imagecreatetruecolor(200, 200);
                        imagecopyresampled($thumbImage, $sourceImage,
                            0, 0, // Destino x, y
                            $cropX, $cropY, // Origen x, y (centro de la imagen)
                            200, 200, // Tamaño final deseado
                            $cropSize, $cropSize // Tamaño del área a copiar (cuadrado)
                        );
                        
                        // Guardar ambas versiones
                        $success = true;
                        if(!imagejpeg($largeImage, "images/usuarios/full/".$idUsuarioActual.".jpg", 90)) {
                            $success = false;
                        }
                        if(!imagejpeg($thumbImage, "images/usuarios/thumbs/".$idUsuarioActual.".jpg", 85)) {
                            $success = false;
                        }
                        ...
                        if (!$success) {
                            ...
                            echo "<div class='alert alert-danger'>Error al guardar la imagen: " . $errorMessage . "</div>";
                        }
                        ...
                        echo "<div class='alert alert-danger'>No se pudo crear la imagen desde el archivo subido.</div>";
                    }
                } else {
                    echo "<div class='alert alert-danger'>El archivo subido no es una imagen válida. Por favor, sube un archivo JPG, PNG o GIF.</div>";
                }
                ...
                $errorMessage = isset($errorMessages[$_FILES['archivo']['error']]) ? 
                               $errorMessages[$_FILES['archivo']['error']] : 
                ...
                <div class="tab-content">
                    <div id="general" class="tab-pane fade in active">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-2">
                                ...
                                <select name="pais" class="form-control">
                                    <option value="">Sin especificar</option>
                                    <option value="593" selected >Ecuador</option>}
                                    option
                                </select>
                            </div>
                            <div class="col-sm-2">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <strong>URL</strong><input name="url2" type="text" id="url2" value="<?=$general_url2; ?>" class="form-control" />
                            </div>
                            <div class="col-sm-3">
                                <strong>Foto de perfil (.jpg, .png o .gif)</strong>
                                <div style="display: flex; align-items: start;">
                                    <input name="archivo" type="file" id="archivo" class="form-control" accept="image/*" onchange="previewImage(this)" style="width: 85%;" />
                                    <button type="button" onclick="cancelImage()" id="cancelButton" class="btn btn-danger btn-sm" style="margin-left: 5px; display: none;"><i class="fas fa-times"></i></button>
                                </div>
                                <input type="hidden" name="funcion" value="<?php echo isset($_REQUEST['id']) ? 'actualizar' : 'insertar'; ?>" />
                            </div>
                            <div class="col-sm-1">
                                <div id="imagePreview" style="width: 80px; height: 80px; border: 1px solid #ddd; border-radius: 4px; text-align: center; overflow: hidden; cursor: pointer;" onclick="showImageModal(this.querySelector('img'))">
                                    <?php if(file_exists("images/usuarios/thumbs/".$idUsuarioActual.".jpg")) { ?>
                                        <img 
                                            src="images/usuarios/thumbs/<?php echo $idUsuarioActual; ?>.jpg?t=<?php echo time(); ?>" 
                                            data-full-src="images/usuarios/full/<?php echo $idUsuarioActual; ?>.jpg?t=<?php echo time(); ?>" 
                                            style="width: 80px; height: 80px; object-fit: cover;" 
                                        />
                                    <?php } else { ?>
                                        <img src="images/consultores/desconocido.jpg" style="width: 80px; height: 80px; object-fit: cover;" />
                                    <?php } ?>
                                </div>
                                ...
                                <div class="modal fade image-modal" id="imageModal" tabindex="-1" role="dialog">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <button type="button" class="close-button" data-dismiss="modal" aria-label="Cerrar">
                                                <i class="fas fa-times"></i>
                                            </button>
                                            <img id="modalImage" src="" alt="Imagen de perfil" />
                                        </div>
                                    </div>
                                </div>
                                <script>
                                // Prevenir reenvío de formulario al recargar
                                if (window.history.replaceState) {
                                    window.history.replaceState(null, null, window.location.href);
                                }

                                function showImageModal(img) {
                                    // Usar la versión en alta calidad si está disponible
                                    const fullSrc = img.getAttribute('data-full-src');
                                    document.getElementById('modalImage').src = fullSrc || img.src;
                                    ...
                                    $('#imageModal').on('click', function(e) {
                                        if (!$(e.target).is('#modalImage') && !$(e.target).is('.close-button') && !$(e.target).is('.fa-times')) {
                                ...
                                function previewImage(input) {
                                    ...
                                    if (input.files && input.files[0]) {
                                        ...
                                        reader.onload = function(e) {
                                            preview.innerHTML = '<img src="' + e.target.result + '" style="width: 80px; height: 80px; object-fit: cover;" />';
                                ...
                                function cancelImage() {
                                    ...
                                    var preview = document.getElementById('imagePreview');
                                    <?php if(file_exists("images/usuarios/".$idUsuarioActual.".jpg")) { ?>
                                        preview.innerHTML = '<img src="images/usuarios/<?php echo $idUsuarioActual; ?>.jpg?t=<?php echo time(); ?>" style="width: 80px; height: 80px; object-fit: cover;" />';
                                    <?php } else { ?>
                                        preview.innerHTML = '<img src="images/consultores/desconocido.jpg" style="width: 80px; height: 80px; object-fit: cover;" />';
                                    <?php } ?>
                                }
                                </script>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-2">
                                    ...
                                    <select name="empresa_paisid" class="form-control">
                                        <option value="">Sin especificar</option>
                                        <option value="282" selected>Ecuador</option>
                                    </select>
                                </div>
                                <div class="col-sm-2">
...
<!-- Información no visible en Crear usuario -> información organizacional
                                <div class="col-sm-3">
                                    <strong>Nombre del socio:</strong>
                                    <input name="empresa_socio" readonly type="text" id="empresa_socio" maxlength="50" value="Confraternidad Carcelaria de Ecuador" class="form-control" />
                                </div> -->
                            </div>
                            <div class="form-group">
                                <div class="col-sm-1"></div>
                                                ...
                                                <h5 style="margin-bottom: 0px;" class="alert alert-info"><?php
                                                    ...
                                                    switch($PSN2->f("principal")){
                                                        ...
                                                        case 3:
                                                            ...
                                                            break;
                                                        case 4:
                                                            echo "Reportes/Capacitadores";
                                                            break;
                                                        case 5:
                                                            echo "Reportes/Facilitadores";
                                                            break;
                                                        case 6:
...
// Se comentó para deshabilitar la opción de incluir la información de regional en Crear usuario -> Información organizacional
//        recargaListaZona();
